...
final_index = os.path.join(src_dir,"synergy_score/final_index.csv")
update_xy = True
old_x = os.path.join(src_dir,"synergy_score/x.npy")
old_x_lengths = os.path.join(src_dir,"synergy_score/old_x_lengths.pkl")
old_y = os.path.join(src_dir,"synergy_score/y.pkl")
...
neural_fp = True
chemfp_drug_feature_file = os.path.join(src_dir, 'chemicals', 'drug_features_all_three_tanh.csv')
...
L1000_downregulation = os.path.join(src_dir, 'F_repr', 'sel_F_drug_sample_1.csv')
add_single_response_to_drug_target = True
F_cl = os.path.join(src_dir, 'F_repr', 'sel_F_cl_sample.csv')
single_response = os.path.join(src_dir, 'chemicals', 'single_response_features.csv')

drug_ECFP = os.path.join(src_dir, 'chemicals', 'ECFP6.csv')
drug_physicochem = os.path.join(src_dir, 'chemicals', 'physicochemical_des.csv')
cl_ECFP = os.path.join(src_dir, 'RF_features', 'features_importance_df.csv')
cl_physicochem = os.path.join(src_dir, 'RF_features', 'features_importance_df_phychem.csv')
inchi_merck = os.path.join(src_dir, 'chemicals', 'inchi_merck.csv')

# networks: string_network, all_tissues_top
network_update = True
network_prop_normalized = True
network_path = os.path.join(src_dir, 'network')
network = os.path.join(src_dir, 'network', 'string_network')
network_matrix = os.path.join(src_dir, 'network', 'string_network_matrix.csv')
split_random_seed = 3
index_in_literature = True
index_renewal = True
train_index = os.path.join(src_dir, 'train_index_' + str(split_random_seed))
test_index = os.path.join(src_dir, 'test_index_' + str(split_random_seed))

renew = False
gene_expression_simulated_result_matrix = os.path.join(src_dir, 'chemicals', 'gene_expression_simulated_result_matrix_string.csv')
random_walk_simulated_result_matrix = os.path.join(src_dir, 'chemicals', 'random_walk_simulated_result_matrix_2401_0.3_norm_36_whole_network_no_mean')
intermediate_ge_target0_matrix = os.path.join(src_dir, 'chemicals', 'intermediate_ge_target0_matrix')
...
processed_expression_raw = os.path.join(src_dir, 'Gene_expression_raw', 'processed_expression_raw_norm')

combine_drug_target_renew = False
combine_drug_target_matrix = os.path.join(src_dir, 'chemicals', 'combine_drug_target_matrix.csv')

drug_profiles_renew = False
drug_profiles = os.path.join(src_dir, 'chemicals','new_dedup_drug_profile.csv')
